...
class PSO:
    ...
    def plot_state_solution(self, min_state):
        ...
        ellipse_target = Ellipse(xy=(0, 0), width=2*(rm * 1e-3 + 100), height=2*(rm * 1e-3 + 100),
...
class PSOStandard(PSO):
    ...
    def optimize(self):
        ...
        while iteration < self.max_iteration:
            ...
            self.position = np.clip(self.velocity + self.position, np.array(self.range_var)[:, 0],
...
class APSO(PSOStandard):
    ...
    def optimize(self):
        ...
        while iteration < self.max_iteration:
            ...
            cognitive_comp = self.c1 * np.diag(r[:, 0]) @ (self.pbest_position - self.position)
            social_comp = self.c2 * np.diag(r[:, 1]) @ (gbest - self.position)
            modified_comp = W * self.c1 / self.c2 * (gbest - self.pbest_position)
            self.velocity = W * self.velocity + cognitive_comp + social_comp + modified_comp
            self.position = np.clip(self.velocity + W * self.position, np.array(self.range_var)[:, 0],
                                    np.array(self.range_var)[:, 1])
            W = self.w1 - (self.w1 + self.w2) * iteration / self.max_iteration
            self.evol_best_fitness[iteration] = self.gbest_fitness_value
            self.evol_p_fitness[:, iteration] = self.pbest_fitness_value
